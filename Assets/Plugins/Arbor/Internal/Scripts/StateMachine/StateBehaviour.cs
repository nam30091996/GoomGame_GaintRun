...
namespace Arbor
{
	...
	public class StateBehaviour : NodeBehaviour
	{
			...
			set
			{
				if (_BehaviourEnabled != value)
				{
					...
					if (stateMachine.currentState == state)
					{
						...
						{
							ActivateInternal(true, !isCalledActivate);
						}
						else
						{
		...
		internal void CallLateUpdateInternal()
		{
			if (!enabled || !behaviourEnabled)
			{
				return;
		...
		public void RebuildStateLinkCache()
		{
			...
			EachField<StateLink>.Find(this, this.GetType(), (StateLink stateLink) =>
			{
				_StateLinkCache.Add(stateLink);
			});
		}

		internal override void OnRebuildFields()
		{
