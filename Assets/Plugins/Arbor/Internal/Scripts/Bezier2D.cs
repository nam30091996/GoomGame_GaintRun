...
namespace Arbor
{
	...
	public sealed class Bezier2D
	{
		...
		public float LinearToInterpolationParam(float t)
		{
			...
			int k = 0;
			for (k = 0; k < kCalculateLengthSplitNum; k++)
			{
				if (_SplitLength[k] <= t && t <= _SplitLength[k + 1])
					break;
			}
			if (k < kCalculateLengthSplitNum)
			{
		...
		public bool Equals(Bezier2D bezier)
		{
			...
			return bezier._StartPosition == _StartPosition &&
					bezier._StartControl == _StartControl &&
					bezier._EndPosition == _EndPosition &&
					bezier._EndControl == _EndControl;
		}
