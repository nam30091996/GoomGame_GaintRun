...
namespace Arbor.BehaviourTree
{
	...
	public abstract class TreeNodeBase : Node
	{
		...
		public TreeNodeBase parentNode
		{
			get
			{
				if (!HasParentLinkSlot())
				{
					return null;
				}

				NodeLinkSlot parentSlot = GetParentLinkSlot();
				if (parentSlot == null)
				{
					return null;
				}

				NodeBranch branch = behaviourTree.nodeBranchies.GetFromID(parentSlot.branchID);
				if (branch == null)
				{
					return null;
				}
...
#endif
		public TreeNodeBase(NodeGraph nodeGraph, int nodeID)
...
#endif
		public abstract bool HasParentLinkSlot();
...
#endif
		public virtual NodeLinkSlot GetParentLinkSlot()
		{
			return null;
		}

		internal void ConnectParentLinkSlot(int branchID)
		{
			if (!HasParentLinkSlot())
			{
				return;
			...
			NodeLinkSlot parentLink = GetParentLinkSlot();
			if (parentLink == null)
			{
				return;
			}

			NodeBranch oldBranch = behaviourTree.nodeBranchies.GetFromID(parentLink.branchID);
			if (oldBranch != null)
			{
				behaviourTree.DisconnectBranch(oldBranch);

				oldBranch = null;
				parentLink.branchID = 0;
			}
			parentLink.branchID = branchID;
		}

		internal void DisconnectParentLinkSlot(int branchID)
		{
			if (!HasParentLinkSlot())
			{
				return;
...
#endif
		public abstract bool HasChildLinkSlot();

		internal virtual void ConnectChildLinkSlot(int branchID)
		{
		}

		internal virtual void DisconnectChildLinkSlot(int branchID)
		{
		}

		internal virtual int OnCalculateChildPriority(int priority)
		{
			return priority;
		}
		...
		internal int CalculatePriority(int priority)
		{
			...
			if (HasChildLinkSlot())
			{
				priority = OnCalculateChildPriority(priority);
			}
