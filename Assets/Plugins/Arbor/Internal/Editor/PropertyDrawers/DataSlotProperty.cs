...
namespace ArborEditor
{
	...
	public class DataSlotProperty : IPropertyChanged
	{
		...
		private DataSlotField _DataSlotField = null;
		...
		public DataSlotField dataSlotField
		{
			get
			{
				if (_DataSlotField == null)
				{
					DataSlot slot = this.slot;
					NodeBehaviour nodeBehaviour = property.serializedObject.targetObject as NodeBehaviour;
					if (slot != null && nodeBehaviour != null)
					{
						_DataSlotField = nodeBehaviour.GetDataSlotField(slot, true);
						EnableCallback();
					}
				}

				return _DataSlotField;
			}
		}
