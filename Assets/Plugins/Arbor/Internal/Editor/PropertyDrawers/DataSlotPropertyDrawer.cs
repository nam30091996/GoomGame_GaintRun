	...
	internal class DataSlotPropertyDrawer : PropertyDrawer
	{
		...
		private static ExternalFields GetExternalFields(System.Type fieldType)
		{
			...
			if (!s_ExternalFields.TryGetValue(elementType, out externalFields))
			{
				...
				foreach (var fieldInfo in elementType.GetFields(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Public))
				{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			using (new ProfilerScope("DataSlotPropertyDrawer.OnGUI"))
			{
				if (ArborEditorWindow.isInNodeEditor)
				{
					...
					slotPosition.height = DataSlotGUI.kSlotHeight;
							...
							case SlotType.Input:
								switch (ArborSettings.dataSlotShowMode)
								{
									...
									case DataSlotShowMode.Flexibly:
										BehaviourEditorGUI.AddInputSlotLink(slotPosition, property);
									...
									case DataSlotShowMode.Inside:
										{
											...
											if (slotGUI != null)
											{
												slotGUI.DoGUI(slotPosition, property, label, new Vector2(-slotPosition.position.x, 0f));
											}
										}
										break;
								}
							...
							case SlotType.Output:
								{
									...
									if (slotGUI != null)
									{
										slotGUI.DoGUI(EditorGUI.IndentedRect(slotPosition), property, label, Vector2.zero);
									}
								}
								break;
						}
...
#endif
				height += DataSlotGUI.kSlotHeight;
