	...
	internal sealed class EnumListParameterEditor : ListParameterEditor
	{
		protected override void OnDrawElement(Rect rect, int index, bool isActive, bool isFocused)
		{
			...
			else
			{
				...
				{
					enumValue = EditorGUITools.EnumMaskField(rect, EditorGUITools.GetTextContent(valueProperty.displayName), (System.Enum)enumValue);
				}
				else
				{
					enumValue = EditorGUI.EnumPopup(rect, EditorGUITools.GetTextContent(valueProperty.displayName), (System.Enum)enumValue);
				}
