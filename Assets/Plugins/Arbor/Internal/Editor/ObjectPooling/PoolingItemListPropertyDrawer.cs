...
namespace ArborEditor.ObjectPooling
{
	...
	internal sealed class PoolingItemListGUI : PropertyEditor
	{
		...
		void DoElementGUI(SerializedProperty property)
		{
			...
			GUIContent amountContent = EditorGUITools.GetTextContent(itemProperty.amount.displayName);
			_LayoutArea.PropertyField(itemProperty.amount, amountContent, true);
