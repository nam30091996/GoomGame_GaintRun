...
namespace Arbor.StateMachine.StateBehaviours
{
	...
	public sealed class TweenTextureOffset : TweenBase
	{
		...
		protected override void OnTweenBegin()
		{
			...
			_Block = RendererPropertyBlock.Get(_CachedTarget);

			_Block.Update();

			Vector4 texST = Vector4.zero;
			if (!_Block.HasProperty(_CachedPropertyID))
			{
				texST = _CachedTarget.sharedMaterial.GetVector(_CachedPropertyID);
			}
			else
			{
				texST = _Block.GetVector(_CachedPropertyID);
			}

			Vector2 startOffset = new Vector2(texST.z, texST.w);
		...
		protected override void OnTweenUpdate(float factor)
		{
			...
			_Block.SetVector(_CachedPropertyID, texST);
